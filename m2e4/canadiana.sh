#! /bin/bash
pages=$(curl 'http://search.canadiana.ca/search?q=ottawa*&field=&so=score&df=1800&dt=1802&fmt=json' | jq '.pages')
...
do
        curl 'http://search.canadiana.ca/search/'${i}'?q=ottawa*&field=&so=score&df=1800&dt=1802&fmt=json' | jq '.docs[] | {key}' >> results.txt
done
